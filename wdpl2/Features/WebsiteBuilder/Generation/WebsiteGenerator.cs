    ...
    public sealed partial class WebsiteGenerator
    {
        ...
        public Dictionary<string, string> GenerateWebsite(bool previewEntryForms = false)
        {
            ...
            if (_settings.ShowLiveScores)
                files["live.html"] = GenerateLiveScoresPage(season, template);
...
@media (max-width: 480px) {
    ...
    .page-canvas .fixture-item { padding: 14px 16px !important; }
            ...
            // Live scores strip sits in normal flow below the absolutely-positioned
            // canvas blocks so it can never overlap them.
            AppendHomeLiveScoresWidget(html);
...
(function(){
    ...
    window.addEventListener('resize',adjustCanvas);
            ...
            List<Player> players, List<Team> teams, int topAdvance)
        {
            ...
            // A group drawn out and played as a knockout has no table, so who
            // went through is read off the tree. It may be two people and no
            // single winner: a group that stops when two are left has no final.
            var isKnockout = KnockoutGroup.IsKnockout(group);
            ...
            // A knockout group shows who came through and nothing else. The ties
            // themselves belong on the card that was played, not on a page whose
            // job is to say where the competition stands.
            if (group.Matches.Any(m => m.IsComplete) && !isKnockout)
            {
