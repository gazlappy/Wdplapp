...
(function() {
    ...
    /// <summary>
    /// Captains Area landing page (captains.html). The PIN-gated client-side
    /// dashboard has been retired in favour of the server-backed Captain Portal
    /// (the PHP/HTML app deployed alongside the league site by
    /// <see cref="Wdpl2.Services.Cloud.BackendDeployService"/>). This page now
    /// just shows a welcome card with a link to that portal.
    /// </summary>
        ...
        Action<StringBuilder> appendFooter)
    {
        ...
        html.AppendLine("                <p style=\"margin-top:16px;\">Captains now use a dedicated portal to enter scorecards live, agree on results with the opposing captain and view their match history.</p>");
        html.AppendLine("                <p style=\"text-align:center; margin:24px 0;\">");
        html.AppendLine("                    <a id=\"captain-portal-link\" href=\"../captain/\" class=\"captain-btn\" style=\"display:inline-block; padding:12px 28px; text-decoration:none;\">Open Captain Portal &#8594;</a>");
        html.AppendLine("                </p>");
        html.AppendLine("                <p class=\"captain-hint\">Sign in with the team username and password issued by your league administrator.</p>");
        html.AppendLine("            </div>");
        html.AppendLine("        </div>");
        html.AppendLine("    </div>");
        ...
        // Best-effort: if the portal isn't at ../captain/ (e.g. league site is at the
        // FTP root rather than a sub-folder) fall back to /captain/.
        html.AppendLine(@"    <script>
(function(){
    var link = document.getElementById('captain-portal-link');
    if (!link) return;
    var segments = window.location.pathname.split('/').filter(Boolean);
    if (segments.length <= 1) link.setAttribute('href', '/captain/');
})();
    </script>");
