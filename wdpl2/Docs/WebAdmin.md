...
**Compare current website revision** reads `admin/sync-current.php` and displays the latest journal payload separately, including during a started decision. The queued revision, request identity and cursors are untouched. For scorecards the endpoint also compares the live card under a row lock and reports divergence or a missing card. An explicit deletion journal payload matches only an absent live card. This comparison is a point-in-time read, not an acknowledgment or recovery action.

Legacy live-card initialization, captain edits/finalization, admin reopening/clearing/reset/deletion, fixture swap/deletion and player-merge card rewrites now use the shared journal lock. Domain writes and journal receipts commit or roll back together. Finalization includes the inbox submission in that transaction. Player merges increment affected live versions and clear prior finalizations. Captain authentication remains separate from admin authentication; journal storage does not grant access. All backend files must be deployed together, including `_scorecard_journal.php`.

Reset/deletion records use an explicit `deleted: true` payload for comparison only. They cannot be applied as empty local results. Historical divergence is not backfilled or automatically repaired. The immutable `submit-result.php` inbox contract is unchanged: independent captain submissions do not replace the shared live card. Legacy requests have no stable client request ID, so this integration does not promise exactly-once retries; inspect the current state after an uncertain response, especially before retrying finalization or fixture swaps.

Server-revision reconciliation and recovery of conflicts after a decision has started are still pending. This increment adds journal coverage and comparison, not conflict resolution. Missing local forms/entries and unknown fixtures remain blocked; do not delete the review queue to bypass them. A browser confirmation updates review metadata only: it does not register a team/player or mean that desktop synchronization is complete.
...
An isolated Docker test setup and staging checklist are in `tools/backend-tests/README.md` at the repository root. It publishes no host database ports, mounts the workspace read-only, and fails if backend prerequisites or journal integration are skipped. The setup must still be executed on a machine with Docker; source/browser checks are not a substitute for PHP/MySQL execution.

The app deployment manifest includes `_scorecard_journal.php`; missing packaged files now stop deployment before any upload. FTP deployment is still non-atomic, so use a maintenance window and keep the previous complete backend for rollback.

Release blocker: an already-desktop scorecard acceptance can return without persisting a request-specific acknowledgement receipt. A lost response followed by another server edit cannot yet be safely recovered from comparison alone. Durable outcome tracking and started-decision recovery must be completed and tested before this is described as production-ready two-way synchronization. Existing pending requests must remain intact.
