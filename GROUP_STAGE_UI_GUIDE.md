# Group Stage UI Implementation Guide

## ?? Complete UI Integration for CompetitionsPage

This guide shows how to add full group stage support to the CompetitionsPage UI.

---

## ?? Changes Required to CompetitionsPage.xaml.cs

### 1. **Add Group Stage Settings UI to `ShowCompetitionEditor()`**

Add this section after the "Bracket Generation" section:
...
### 4. **Add `CreateGroupContent()` Method**
...
### 5. **Add `CreateStandingsTable()` Method**
...
### 6. **Add `CreateGroupMatchCard()` Method**
...
### 7. **Add `ApplyAllGroupScores()` Method**
...
3. ? `CreateGroupContent()` - Build group view (standings + matches)
4. ? `CreateStandingsTable()` - Display group standings table
5. ? `CreateGroupMatchCard()` - Display group match with scores
6. ? `ApplyAllGroupScores()` - Process all group match scores
7. ? `OnFinalizeGroups()` - Create knockouts from group results

### UI Components Added:
- ? Group stage settings form
- ? Generate Groups button
- ? View Groups button
...
### Test Scenario: 128-Player Singles Tournament

1. **Create Competition**
   - Name: "Winter Championship 2025"
   - Format: "SinglesGroupStage"
...
4. **Generate Groups**
   ...
   - 8 groups created (A-H)
   - 16 players per group
   - 120 matches per group
...
6. **Finalize**
   ...
   - Main bracket: 16 players (top 2 from each group)
   - Plate bracket: 16 players (next 2 from each group)

7. **Play Knockouts**
   - Main competition shows Round of 16 bracket
   - Plate competition shows separate Round of 16 bracket
