...
- Entry forms use private editor drafts and shared rendering in `WebsiteGenerator.EntryForms.cs` for public pages and non-submitting previews. `EntryFormRules` centralizes validation and inclusive closing dates. Public delivery uses a credential-free HTTPS POST with an explicit matching acknowledgement; private collection tokens never enter generated HTML. Without a compatible endpoint, forms explicitly download for manual delivery, never claim browser storage is a submission. Own-hosting mode uses `HostedEntryFormsService`, Web Inbox HTTPS/admin credentials and the bundled `api/entry-forms/submit.php` receiver; publish the complete saved form definitions explicitly before deploying the generated site. Server validation uses Europe/London deadlines, atomic idempotent storage, exact-origin CORS and private paginated collection. Omitted forms close without deleting history. See `wdpl2/Docs/OnlineForms.md`; the older PHP captain/results inbox remains a separate contract. Imports require explicit form identity and review, preserve historical field values, and link season teams only by explicit records.
